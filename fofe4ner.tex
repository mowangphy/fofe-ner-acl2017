...
NER and MD is a task of identifying entities (name and/or nominal) from raw text, and classifying the detected entities into one of the pre-defined categories such as person, organization, location, etc. Some tasks focus on named entities only, 
%for example, 
%\begin{quote}
%	\label{emp:4types}
%	\small
...
\newcite{kim2015character} models morphology in the character level since this may provide some additional advantages in dealing with unknown or out-of-vocabulary (OOVs) words in a language.
...
In our systems, we treat all nominal mentions as special types of named entities and both named and nominal entities are recognized using one model. Here we have broken down the system performance according to different languages and categories of entities (named or nominal). Our system, achieves 0.718 in $F_1$ score in the KBP2016 trilingual EDL track, which ranks second among all participants. Note that our result is produced by a single system while the top system is emsembled by two different models, each of which is from 5-fold cross-validation.


\section{Conclusion}

In this paper, we propose a novel and simple solution to NER and MD by applying FFNN on top of FOFE-based features.
This simple approach achieves performance very close to state-of-the-art on various NER and MD tasks, without using any external knowledge or feature engineering.
